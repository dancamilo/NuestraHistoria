...
function initCarousel() {
  ...
  if (!track || !dotsWrap || fotos.length === 0) return;
  ...
  fotos.forEach((foto, i) => {
    ...
    if (img) {
      img.addEventListener("error", () => {
        console.error("No se pudo cargar la imagen:", foto.src);
...
function initSongs() {
  ...
  if (!notifyBtn || !addBtn || !log) return;
  ...
  addBtn.addEventListener("click", () => {
    db.collection("songs")
      ...
      .catch((error) => {
        console.error("ERROR FIREBASE - GUARDAR SONG:", error);
  ...
  db.collection("songs")
    ...
    .onSnapshot(
      ...
      (error) => {
        console.error("ERROR FIREBASE - SONGS:", error);
        toast("Error al cargar las canciones: " + error.message);
      }
    );
...
function initTimeline() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    db.collection("timeline")
      ...
      .then(() => {
        form.reset();
        toast("Fecha añadida correctamente 💙");
      })
      .catch((error) => {
        console.error("ERROR FIREBASE - GUARDAR TIMELINE:", error);
        toast("No se pudo guardar la fecha.");
      });
  });
    ...
    .onSnapshot(
      (snapshot) => {
        ...
        snapshot.forEach((doc) => {
          ...
          dateEl.textContent = formatFecha(fecha);
      ...
      (error) => {
        console.error("ERROR FIREBASE - TIMELINE:", error);
        toast("Error al cargar la historia: " + error.message);
      }
    );
}
...
function initGrowth() {
  const tituloEl = document.getElementById("pageElTitulo");
  const tituloElla = document.getElementById("pageEllaTitulo");

  if (tituloEl) {
    tituloEl.textContent = CONFIG.nombreEl;
  }

  if (tituloElla) {
    tituloElla.textContent = CONFIG.nombreElla;
  }

  document
    .querySelectorAll('#creciendo form[data-persona]')
    .forEach((form) => {
      form.addEventListener("submit", (e) => {
        e.preventDefault();
        ...
        const defecto = form
          .querySelector(".growth-defecto")
          .value
          .trim();

        const compromiso = form
          .querySelector(".growth-compromiso")
          .value
          .trim();
    ...
    .onSnapshot(
      (snapshot) => {
        ...
        snapshot.forEach((doc) => {
          ...
          const defectoEl = document.createElement("span");
          defectoEl.className = "defecto";
          defectoEl.textContent = d.defecto || "";

          const compromisoEl = document.createElement("span");
          compromisoEl.className = "compromiso";
          compromisoEl.textContent = d.compromiso || "";

          li.append(defectoEl, compromisoEl);
      ...
      (error) => {
        console.error("ERROR FIREBASE - GROWTH:", error);
        toast("Error al cargar crecimiento: " + error.message);
      }
    );
}
...
function initDreams() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    db.collection("dreams")
      ...
      .catch((error) => {
        console.error("ERROR FIREBASE - GUARDAR DREAM:", error);
        toast("No se pudo guardar el sueño.");
      });
  });
    ...
    .onSnapshot(
      (snapshot) => {
        ...
        snapshot.forEach((doc) => {
          ...
          li.className =
            "dream-item" + (d.cumplido ? " achieved" : "");

          const input = document.createElement("input");
          input.type = "checkbox";
          input.checked = d.cumplido === true;
          ...
          input.addEventListener("change", (ev) => {
            db.collection("dreams")
              .doc(doc.id)
              .update({
                cumplido: ev.target.checked
              })
              .then(() => {
                toast("Sueño actualizado ✓");
              })
              .catch((error) => {
                console.error(
                ...
                toast("No se pudo actualizar el sueño.");
              });
          });
      ...
      (error) => {
        console.error("ERROR FIREBASE - DREAMS:", error);
        toast("Error al cargar los sueños: " + error.message);
      }
    );
}
  ...
  form.addEventListener("submit", (e) => {
    ...
    db.collection("todos")
      ...
      .catch((error) => {
        ...
        toast("No se pudo guardar el pendiente.");
      });
  });
    ...
    .onSnapshot(
      (snapshot) => {
        ...
        snapshot.forEach((doc) => {
          ...
          check.type = "checkbox";
          check.checked = d.hecho === true;

          check.addEventListener("change", (ev) => {
            db.collection("todos")
              .doc(doc.id)
              .update({
                hecho: ev.target.checked
              })
              .then(() => {
